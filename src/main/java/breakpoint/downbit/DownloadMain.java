package breakpoint.downbit;

import breakpoint.downbit.thread.DownloadThread;
import breakpoint.downbit.thread.LogThread;
import breakpoint.downbit.util.FileUtils;
import breakpoint.downbit.util.HttpUtls;
import breakpoint.downbit.util.LogUtils;
import breakpoint.downbit.util.ThunderUtils;
...
//        String name = scan.nextLine();
    ...
    public void download(String url) throws Exception {
        ...
        // 开始下载
        /**
         * 通过Future.get()方法使得子线程和主线程有序执行。
         * executor.submit()时各线程已经在执行，这里Future.get()会阻塞线程(wait)等待返回，wait状态不影响其他线程执行
         * 主要目的是为了主线程的merge()时其他分片进程完成
         */
